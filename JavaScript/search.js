      ...
      fetch(`https://jsonplaceholder.typicode.com/users?name=${searchPhrase}`)
        ...
        .then((usersByName) => {
          if (usersByName.length > 0) {
            ...
            fetch(
              `https://jsonplaceholder.typicode.com/users?email=${searchPhrase}`
            )
              .then((res) => res.json())
              .then((usersByEmail) => {
...
searchPageForm.addEventListener("submit", (event) => {
  ...
  fetch(
    `https://jsonplaceholder.typicode.com/users?username_like=${searchInput}`
  )
    .then((res) => res.json())
    .then((users) => {
      if (users.length > 0) {
        ...
        fetch(
          `https://jsonplaceholder.typicode.com/users?name_like=${searchInput}`
        )
          .then((res) => res.json())
          .then((usersByName) => {
            if (usersByName.length > 0) {
              ...
              usersByName.map((user) => {
                let userItem = document.createElement("li");
                userItem.classList.add("result-link");
                userItem.innerHTML = `<a href="./user.html?user_id=${user.id}">${user.name}</a>`;
              ...
              fetch(
                `https://jsonplaceholder.typicode.com/users?email_like=${searchInput}`
              )
                .then((res) => res.json())
                .then((usersByEmail) => {
                  if (usersByEmail.length > 0) {
